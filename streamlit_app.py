# streamlit_app.py (FINAL DIAGNOSTIC VERSION)
...
class AlphaVantageTools(Toolkit):
    # (The AlphaVantageTools class is perfect, no changes needed)
...
def get_financial_agent():
    # (The simplified agent definition is perfect, no changes needed)
    model_id = "llama-3.3-70b-versatile"
    financial_agent = Agent(name="Financial AI Agent", role="You are a world-class financial analyst. Your goal is to help users by accessing financial data and web search.", model=Groq(id=model_id, api_key=groq_api_key), tools=[AlphaVantageTools(), DuckDuckGo()], instructions=["First, understand the user's query.", "Then, choose the best tool to answer their question (either financial data or web search).", "Present the answer clearly to the user, using tables and formatting where appropriate.", "If the user's query is unclear, ask for clarification."], markdown=True)
    ...
    if prompt := st.chat_input("Ask me about stocks, news, and more..."):
        ...
        with st.chat_message("assistant"):
            ...
            # --- THIS IS THE ONLY PART THAT CHANGED ---
                ...
                # Add the specific technical error for debugging
